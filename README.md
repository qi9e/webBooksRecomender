...
Gemini 可能返回格式正确但内容不完全可靠的 JSON，所以后端会再次校验：

- `isbn` 是否真的存在于数据库
- 如果 `isbn` 没命中，再尝试用 `title` 匹配
- 最终只返回数据库里确实存在的书
...
而 CSV 又没有额外的“馆内条码”字段，所以当前实现把 `ISBN` 视为“条码来源字段”：
...
前端使用浏览器的 Web Speech API。不同浏览器支持情况不完全一致，Chrome 系浏览器通常更合适。如果浏览器不支持，系统会自动退回文字输入模式。
...
1. 我可以继续帮你补一个 **示例 `database.csv`**
2. 也可以继续把这个项目扩成 **可直接打包上线的完整版本**
# webBooksRecomender
